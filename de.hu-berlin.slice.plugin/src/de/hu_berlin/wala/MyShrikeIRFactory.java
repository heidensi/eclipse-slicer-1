 ...
 */
public class MyShrikeIRFactory implements IRFactory<IBytecodeMethod> {
  ...
  public IR makeIR(final IBytecodeMethod method, Context C, final SSAOptions options) throws IllegalArgumentException {
    ...
    return new IR(method, newInstrs, symbolTable, newCfg, options) {
      ...
      {
        MySSABuilder builder = MySSABuilder.make(method, newCfg, shrikeCFG, newInstrs, symbolTable, buildLocalMap, options
            .getPiNodePolicy());
        builder.build();
        if (buildLocalMap)
